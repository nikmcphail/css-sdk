...
public:
  ...
  virtual qangle_t const&         get_render_angles(void)                                 = 0;
  ...
  virtual void setup_weights(const matrix_3x4_t* bone_to_world, int flex_weight_count,
                             float* flex_weights, float* flex_delayed_weights)            = 0;
  virtual void do_animation_events(void)                                                  = 0;
  virtual ipvs_notify_t* get_pvs_notify_interface()                                       = 0;
  ...
  virtual void           get_shadow_render_bounds(vector3_t& mins, vector3_t& maxs,
                                                  shadow_type_e shadow_type)              = 0;
  virtual bool           should_receive_projected_textures(int flags)                     = 0;
  virtual bool get_shadow_cast_distance(float* distance, shadow_type_e shadow_type) const = 0;
  virtual bool get_shadow_cast_direction(vector3_t*    direction,
                                         shadow_type_e shadow_type) const                 = 0;
  virtual bool is_shadow_dirty()                                                          = 0;
  virtual void mark_shadow_dirty(bool dirty)                                              = 0;
  virtual i_client_renderable_t*  get_shadow_parent()                                     = 0;
  virtual i_client_renderable_t*  first_shadow_child()                                    = 0;
  virtual i_client_renderable_t*  next_shadow_peer()                                      = 0;
  virtual shadow_type_e           shadow_cast_type()                                      = 0;
  virtual void                    create_model_instance()                                 = 0;
  virtual model_instance_handle_t get_model_instance()                                    = 0;
  virtual const matrix_3x4_t&     renderable_to_world_transform()                         = 0;
  virtual int                     lookup_attachment(const char* attachment_name)          = 0;
  virtual bool   get_attachment(int number, vector3_t& origin, qangle_t& angles)          = 0;
  virtual bool   get_attachment(int number, matrix_3x4_t& matrix)                         = 0;
  ...
  virtual bool   ignore_z_buffer(void) const                                              = 0;
};